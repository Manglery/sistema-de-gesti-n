...
class ErrorReporter {
  ...
  private setupConsoleInterceptors() {
    ...
    // Check each method independently to avoid partial-wrap race conditions
    // If already wrapped by immediate interceptors, preserve those wrappers
    if (!currentWarn.__errorReporterWrapped) {
      this.originalConsoleWarn = console.warn;
      const wrappedWarn = this.createConsoleInterceptor(
        "warn",
        this.originalConsoleWarn,
        WARNING_PREFIX
      ) as WrappedConsoleFn;
      wrappedWarn.__errorReporterWrapped = true;
      console.warn = wrappedWarn;
    } else {
      this.originalConsoleWarn = console.warn;
    }

    if (!currentError.__errorReporterWrapped) {
      this.originalConsoleError = console.error;
      const wrappedError = this.createConsoleInterceptor(
        "error",
        this.originalConsoleError,
        CONSOLE_ERROR_PREFIX
      ) as WrappedConsoleFn;
      wrappedError.__errorReporterWrapped = true;
...
const sendImmediateError = async (payload: ImmediatePayload): Promise<void> => {
  ...
  try {
    ...
    // Fail silently (includes timeout aborts)
  } finally {
    clearTimeout(timeoutId);
  }
};
