    ...
    <AppLayout className="bg-slate-50/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="py-8 md:py-10 lg:py-12 space-y-10">
          {/* Header Section with Brand Accents */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <CardHeader className="bg-slate-50/50 border-b p-5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-xs font-black uppercase tracking-tight text-slate-800">Actividad Reciente</CardTitle>
                    </div>
            ...
            <div className="lg:col-span-9 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard title="USUARIOS ACTIVOS" value={activeData?.stats?.usuarios ?? 0} icon={Users} iconColor="text-blue-600 bg-blue-50" subtitle="En las últimas 24h" />
                <StatCard title="PEDIDOS PENDIENTES" value={activeData?.stats?.pendientes ?? 0} icon={Clock} iconColor="text-orange-600 bg-orange-50" trend="Prioridad Alta" trendType="negative" />
                <StatCard title="TOTAL DESPACHOS" value={activeData?.stats?.despachos ?? 0} icon={Truck} trend="+12.4%" trendType="positive" iconColor="text-blue-600 bg-blue-50" subtitle="vs mes anterior" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard title="VALOR INVENTARIO" value={activeData?.stats?.inventario ?? '€0'} icon={Package} iconColor="text-emerald-600 bg-emerald-50" subtitle="Valorización Stock" />
                <StatCard title="EFECTIVIDAD" value={activeData?.stats?.efectividad ?? '0%'} icon={TrendingUp} trend="Meta 95%" trendType="neutral" iconColor="text-orange-600 bg-orange-50" />
                <StatCard title="SALIDA MENSUAL" value={activeData?.stats?.valorSalida ?? '€0'} icon={Wallet} iconColor="text-purple-600 bg-purple-50" trend="En curso" trendType="positive" />
              </div>
